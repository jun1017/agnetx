...
void AgentXSharedTable::remove_row(const Oidx& ind)
{
    ...
    if (r) {
        if (indexStrategy != noIndexAllocation) {
            ...
            if (vbs) {
              backReference->deallocate_index(myContext, vbs, index_len, 
                      this, new Oidx(ind)); 
              delete[] vbs;
            }
        }
...
int SubAgentXMib::open_session() 
{
	if (session) {
		// disconnect
        if (indexRegistry) {
            indexRegistry->remove_session(session->get_id());
        }
		delete session;
	}
...
void SubAgentXMib::do_process_request(Request* req)
{
	...
	case AGENTX_CLEANUPSET_PDU: {
		  ...
		  process_cleanup_set_request(req);
                  // remove request from request list and delete it
	...
	case AGENTX_RESPONSE_PDU: {
		  ...
		  // do not finalize/delete here <-> it´s a response and already 
                  // deleted
		  return;
	}
	...
	if (err == SNMP_ERROR_SUCCESS) {
		...
		if (range.get_lower() <= range.get_upper()) {
                    if (!is_complex_node(entry)) { 
                      ...
                      LOG((nextOid.len() == 0) ? 
                          "not determined" : nextOid.get_printable());
                      LOG_END;
...
void SubAgentXMib::delete_request(Request* req) 
{
	...
	switch (pdu->get_agentx_type()) {
        ...
        // do  not call Mib::delete_request here
}
