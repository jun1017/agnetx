...
{
	AgentXPdu* response = 
           new AgentXPdu(req.network_byte_order(), AGENTX_RESPONSE_PDU);
...
void MasterAgentXMib::process_ax_response(AgentXPdu& response, bool timely)
{
	...
	if (((ax_req->get_agentx_type() == AGENTX_GET_PDU) ||
		...
		while (request) {
                    if (requestList->trylock_request(request) == 
                            Synchronized::BUSY) {
                        s->wait(10);
	...
	Request* request = 
	  requestList->get_request(response.get_transaction_id());
	while (request) {
	    if (requestList->trylock_request(request) == Synchronized::BUSY) {
                s->wait(10);
                request = requestList->get_request(response.get_transaction_id());
	    }
	    else {
                LOG_BEGIN(loggerModuleName, DEBUG_LOG | 8);
                LOG("MasterAgentXMib: try lock request succeeded (tid)(req-id)");
                LOG(response.get_transaction_id());
	...
	if ((reqtype == AGENTX_TESTSET_PDU) ||
	  ...
	  if (request->finished()) {
		Mib::finalize(request);
              ...
              requestList->unlock_request(request);
	  }
	}
}
